...
export const CREW_PLAYBOOK =
  'ROLE: CREWMATE. Goal: identify and vote out the single impostor. SPREAD OUT and explore different rooms — do not all huddle together, or you give the impostor cover. Watch who is alone with whom, remember movements, and REPORT any dead body you find to trigger a meeting. Only call an emergency meeting if you have a strong, specific suspicion. In meetings, share exactly what you saw and vote based on where people were.';

export const IMPOSTOR_PLAYBOOK =
  'ROLE: IMPOSTOR. Killing is your PRIMARY win condition — you win when impostors equal or outnumber the living crew. When you are ALONE in a room with exactly one crewmate and no one else is present, KILL them — do not hesitate, do not keep waiting for a "better" moment. Hunt: move toward rooms where a single crewmate is isolated. Never kill when a third person is watching. Use vents to escape the scene and fake alibis. In meetings, act like an innocent crewmate, lie smoothly, and pin the blame on someone else. Never admit anything.';
